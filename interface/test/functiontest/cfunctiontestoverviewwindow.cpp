...
CFunctionTestOverviewWindow::CFunctionTestOverviewWindow(QWidget *parent) :
    QWidget(parent)
{
    _connected = 0;
...
void CFunctionTestOverviewWindow::_232ButtonClicked()
{
    ...
    if(job!=0)
    {
        ...
        else
        {
            ...
            _232Button->setEnabled(false);
            job->connect232(FUNCTIONTEST_ID);
        }
    }
    else
...
void CFunctionTestOverviewWindow::_485ButtonClicked()
{
    CTestJob* job = ((CApp*)qApp)->_tjob;
    if(job!=0)
    {
        ...
        else
        {
            ...
            job->connect485(FUNCTIONTEST_ID);
        }
    }
    else
        emit newMessage(tr("没有进行作业设置，请先设置"),1);
...
void CFunctionTestOverviewWindow::_232Setup()
{
    ((CApp*)qApp)->_tjob->start232T(_MS_500_);
    _232Button->setEnabled(true);
    _connected |= 0x1;
    if(_connected&1 && _connected&2)
        _nextButton->setEnabled(true);
    _232Button->setText(tr("建立仪表串口连接"));
    emit newMessage(tr("仪表已经设置为功能全检模式"),0);
}

void CFunctionTestOverviewWindow::_485Setup()
{
...
void CFunctionTestOverviewWindow::stop232()
{
    _232Button->setEnabled(true);
    _connected &= ~0x2;
    _nextButton->setEnabled(false);
...
void CFunctionTestOverviewWindow::stop485()
{
    _485Button->setEnabled(true);
    _connected &= ~0x1;
    _nextButton->setEnabled(false);
...
void CFunctionTestOverviewWindow::nextButtonClicked()
{
    emit nextList(-1);
}
