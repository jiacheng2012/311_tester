...
CAdjustTestOilTableWindow::CAdjustTestOilTableWindow(QWidget *parent) :
    QWidget(parent)
{
    //***UI CONPONENTS
    ...
    _savaAllAdjustButton = new QPushButton(tr("保存燃油表调校参数\n(CTRL + S)"));//保存所有调校
    _readButton = new QPushButton(tr("读取燃油表调校参数\n(R)"));
    ...
    //***Layout
    QGroupBox *_centerGroupBox = new QGroupBox(tr("燃油表调校"));
    ...
    _centerGrid->addWidget(new QLabel(tr("理论值 (%)：")),4,0);
    ...
    connect((CApp*)qApp,SIGNAL(sendBackAdjustTestData_oil(CDataFrame)),this,SLOT(sendBackData(CDataFrame)));
    //***
    ...
    for(int i=0;i<cf->value_meter["oil"].size();i++)
    {
        value[i] = cf->value_meter["oil"][i];
        sample[i] = cf->sample_meter["oil"][i];
    }

    updateInterface(cf->value_meter["oil"].size(),value,sample);
}

void CAdjustTestOilTableWindow::zeroButtonClicked()
{
    sendFrame(0x1,0x0,0x0);
    setProperty("zero",1);
    _zeroButton->setEnabled(false);
}

void CAdjustTestOilTableWindow::increaseButtonClicked()
{
    if(property("testFlag").toBool())
    {
        ...
        sendFrame(0x1,0x1,newv);
    }
}

void CAdjustTestOilTableWindow::decreaseButtonClicked()
{
    if(property("testFlag").toBool())
    {
        int newv = _currentStep[property("id").toInt()]->value() - _currentAmount->value();
...
void CAdjustTestOilTableWindow::adjustButtonClicked()
{
    ...
    sendFrame(0x1,0x1,_currentStep[property("id").toInt()]->value());
    _centerGrid->removeWidget(_currentItem);
    _centerGrid->addWidget(_currentItem,3,property("id").toInt()+1);
}

void CAdjustTestOilTableWindow::stepValueChanged(int v)
{
...
void CAdjustTestOilTableWindow::sendFrame(quint8 c,quint8 m,qint16 s)
{
...
void CAdjustTestOilTableWindow::sendBackData(CDataFrame a)
{
            ...
            property("zero").toInt())
    {
        ...
        if(a._command == _METER_FRAME_INIT_PRESPONSE_ADJUST_)
        {
            if(property("read").toInt()==1)
            {
                ...
                int num = rt.num_meter[0];

                adjustSaveParaFrame_0x42_0x45 b={_METER_FRAME_SAVE_PARA_OIL_ADJUST_,0x00};
                b.command = (0x01<<4);
                b.number = num;
            ...
            else if(property("read").toInt()==0)
            {
                ...
                for(int i=0;i<property("number").toInt();i++)
                {
                    cf->value_meter["oil"][i] = _theoryValue[i]->text().toInt();
                    cf->sample_meter["oil"][i] = _sampleValue[i]->text().toInt();
                }
        ...
        else if(a._command == _METER_FRAME_RSAVE_PARA_OIL_ADJUST_)
        {
            if(property("read").toInt()==1)
            {
...
void CAdjustTestOilTableWindow::saveAllAdjustButtonClicked()
{
    setProperty("read",0);
    setProperty("testFlag",0);
    ...
    b.num_meter[0] = property("number").toInt();
...
void CAdjustTestOilTableWindow::writeAdjustInitFrame(adjustInitParaFrame_0x41 &b)
{
...
void CAdjustTestOilTableWindow::writeButtonClicked()
{
    int iRet = QMessageBox::question(this, "警告", "确定把燃油表调校参数（同时包括其他所有调校项目的参数）写入仪表？",
...
void CAdjustTestOilTableWindow::readButtonClicked()
{
    int iRet = QMessageBox::question(this, "警告", "确定把仪表当前燃油表调校参数读入？",
                                     QMessageBox::Yes, QMessageBox::No);
    if(iRet == QMessageBox::No)
        return;
    else
...
void CAdjustTestOilTableWindow::updateInterface(int number,vu16 value,v16 sample)
{
    int i;
...
/*快捷键*/
void CAdjustTestOilTableWindow::keyReleaseEvent(QKeyEvent *event)
{
