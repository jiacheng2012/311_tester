...
CAdjustTestRotateTableWindow::CAdjustTestRotateTableWindow(QWidget *parent) :
    QWidget(parent)
{
    //***UI CONPONENTS
    ...
    _savaAllAdjustButton = new QPushButton(tr("保存转速表调校参数\n(CTRL + S)"));//保存所有调校
    _readButton = new QPushButton(tr("读取转速表调校参数\n(R)"));
    ...
    //***Layout
    QGroupBox *_centerGroupBox = new QGroupBox(tr("转速表调校"));
    ...
    _centerGrid->addWidget(new QLabel(tr("理论值 (转/分)：")),4,0);
    ...
    connect((CApp*)qApp,SIGNAL(sendBackAdjustTestData_rotate(CDataFrame)),this,SLOT(sendBackData(CDataFrame)));
    //***
    cf = ((CApp*)qApp)->_tjob->_aconfig;

    vu16 value,sample;

    for(int i=0;i<cf->value_meter["rspeed"].size();i++)
    {
        value[i] = cf->value_meter["rspeed"][i];
        sample[i] = cf->sample_meter["rspeed"][i];
    }

    updateInterface(cf->value_meter["rspeed"].size(),value,sample);
}

void CAdjustTestRotateTableWindow::zeroButtonClicked()
{
    sendFrame(0x2,0x0,0x0);
    setProperty("zero",1);
    _zeroButton->setEnabled(false);
}

void CAdjustTestRotateTableWindow::increaseButtonClicked()
{
    if(property("testFlag").toBool())
    {
        ...
        sendFrame(0x2,0x1,newv);
    }
}

void CAdjustTestRotateTableWindow::decreaseButtonClicked()
{
    if(property("testFlag").toBool())
    {
        int newv = _currentStep[property("id").toInt()]->value() - _currentAmount->value();
...
void CAdjustTestRotateTableWindow::adjustButtonClicked()
{
    ...
    sendFrame(0x2,0x1,_currentStep[property("id").toInt()]->value());
    _centerGrid->removeWidget(_currentItem);
    _centerGrid->addWidget(_currentItem,3,property("id").toInt()+1);
}

void CAdjustTestRotateTableWindow::stepValueChanged(int v)
{
...
void CAdjustTestRotateTableWindow::sendFrame(quint8 c,quint8 m,qint16 s)
{
...
void CAdjustTestRotateTableWindow::sendBackData(CDataFrame a)
{
            ...
            property("zero").toInt())
    {
        ...
        if(a._command == _METER_FRAME_INIT_PRESPONSE_ADJUST_)
        {
            if(property("read").toInt()==1)
            {
                ...
                int num = rt.num_meter[1];

                adjustSaveParaFrame_0x42_0x45 b={_METER_FRAME_SAVE_PARA_RSPEED_ADJUST_,0x00};
                b.command = (0x01<<4);
                b.number = num;
            ...
            else if(property("read").toInt()==0)
            {
                ...
                for(int i=0;i<property("number").toInt();i++)
                {
                    cf->value_meter["rspeed"][i] = _theoryValue[i]->text().toInt();
                    cf->sample_meter["rspeed"][i] = _sampleValue[i]->text().toInt();
                }
                ...
                if(property("number").toInt()>0)
                {
                    ...
                    b.sample0[0] = tmp[0];
                    b.sample0[1] = tmp[1];
                    tmp = arrangeByte(_theoryValue[0]->text().toInt(),2);
                    b.value0[0] = tmp[0];
                    b.value0[1] = tmp[1];
                }
                if(property("number").toInt()>1)
                {
                    ...
                    b.sample1[0] = tmp[0];
                    b.sample1[1] = tmp[1];
                    tmp = arrangeByte(_theoryValue[1]->text().toInt(),2);
                    b.value1[0] = tmp[0];
                    b.value1[1] = tmp[1];
                }
                if(property("number").toInt()>2)
                {
                    ...
                    b.sample2[0] = tmp[0];
                    b.sample2[1] = tmp[1];
                    tmp = arrangeByte(_theoryValue[2]->text().toInt(),2);
                    b.value2[0] = tmp[0];
                    b.value2[1] = tmp[1];
                }
                if(property("number").toInt()>3)
                {
                    ...
                    b.sample3[0] = tmp[0];
                    b.sample3[1] = tmp[1];
                    tmp = arrangeByte(_theoryValue[3]->text().toInt(),2);
                    b.value3[0] = tmp[0];
                    b.value3[1] = tmp[1];
                }
                if(property("number").toInt()>4)
                {
                    ...
                    b.sample4[0] = tmp[0];
                    b.sample4[1] = tmp[1];
                    tmp = arrangeByte(_theoryValue[4]->text().toInt(),2);
                    b.value4[0] = tmp[0];
                    b.value4[1] = tmp[1];
                }
                if(property("number").toInt()>5)
                {
                    ...
                    b.sample5[0] = tmp[0];
                    b.sample5[1] = tmp[1];
                    tmp = arrangeByte(_theoryValue[5]->text().toInt(),2);
                    b.value5[0] = tmp[0];
                    b.value5[1] = tmp[1];
                }
                if(property("number").toInt()>6)
                {
                    ...
                    b.sample6[0] = tmp[0];
                    b.sample6[1] = tmp[1];
                    tmp = arrangeByte(_theoryValue[6]->text().toInt(),2);
                    b.value6[0] = tmp[0];
                    b.value6[1] = tmp[1];
                }
                if(property("number").toInt()>7)
                {
                    ...
                    b.sample7[0] = tmp[0];
                    b.sample7[1] = tmp[1];
                    tmp = arrangeByte(_theoryValue[7]->text().toInt(),2);
                    b.value7[0] = tmp[0];
                    b.value7[1] = tmp[1];
                }
        ...
        else if(a._command == _METER_FRAME_RSAVE_PARA_RSPEED_ADJUST_)
        {
            if(property("read").toInt()==1)
            {
...
void CAdjustTestRotateTableWindow::saveAllAdjustButtonClicked()
{
    setProperty("read",0);
    setProperty("testFlag",0);
...
void CAdjustTestRotateTableWindow::writeAdjustInitFrame(adjustInitParaFrame_0x41 &b)
{
...
void CAdjustTestRotateTableWindow::writeButtonClicked()
{
    int iRet = QMessageBox::question(this, "警告", "确定把转速表调校参数（同时包括其他所有调校项目的参数）写入仪表？",
...
void CAdjustTestRotateTableWindow::readButtonClicked()
{
    int iRet = QMessageBox::question(this, "警告", "确定把仪表当前转速表调校参数读入？",
                                     QMessageBox::Yes, QMessageBox::No);
    if(iRet == QMessageBox::No)
        return;
    else
...
void CAdjustTestRotateTableWindow::updateInterface(int number,vu16 value,vu16 sample)
{
    int i;
...
/*快捷键*/
void CAdjustTestRotateTableWindow::keyReleaseEvent(QKeyEvent *event)
{
