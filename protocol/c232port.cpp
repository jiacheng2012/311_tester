...
C232Port::C232Port()
{
    ...
    //***设置串口类
    _port = new QextSerialPort("COM" + QString::number(cf->_connectParmsMap["meter-portnumber"]+1), QextSerialPort::EventDriven);
/*
...
void C232Port::Send(QByteArray data)
{
    int i = _port->write(data);
}

void C232Port::Receive()
{
    char buff[1024];
...
//  QByteArray tmp(buff);
    ...
    emit newData(tmp);
//    qDebug("received: %d",tmp.size());
//    emit newMessage(QString("received: %1").arg(tmp.size()),0);
}
